...
Pranav Lende is an undergraduate student at the Georgia Institute of Technology. His interests include scientific machine learning, system identification, and building reproducible tools that connect theoretical methods to practical workflows for noisy spatiotemporal data. This project reflects his focus on designing end-to-end research pipelines that are easy for other researchers to run, verify, and extend.

\medskip

Sung Ha Kang is a faculty member at the Georgia Institute of Technology whose research focuses on data-driven discovery of dynamical systems and partial differential equations, with an emphasis on robust identification in noisy and imperfect measurement settings. Her work develops methods that combine principled modeling, numerical optimization, and scientific machine learning to enable reliable equation discovery and inference from real-world data. She also contributes to open and reproducible research through publicly available implementations of her group’s methods.
...
The training pipeline consists of: (1) loading the labeled dataset with features and best-method labels; (2) splitting into 80\% training and 20\% test sets with stratification; (3) standardizing features using z-score normalization (fit on training data only); and (4) training each classifier and evaluating on the held-out test set.
...
\begin{enumerate}
    ...
    \item \textbf{RobustIDENT}: Robust identification using $\ell_1$ loss (least absolute deviation) with $\ell_1$ regularization, solved via ADMM.
\end{enumerate}
...
\noindent\textbf{Code and data repository:} \url{https://github.com/ALLENDE123X/ident-meta}
